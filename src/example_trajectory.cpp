...
int main(int argc, char** argv){
	...
	while(ros::ok()){
		...
		if(lcl_sub_flag){
			geometry_msgs::Point lcl_point;

			lcl_point.x = lcl_odom.pose.pose.position.x;
			lcl_point.y = lcl_odom.pose.pose.position.y;
			lcl_point.z = lcl_odom.pose.pose.position.z;

			cout<<"lcl_point.x : "<<lcl_point.x<<endl;
			cout<<"lcl_point.y : "<<lcl_point.y<<endl;
			cout<<"lcl_point.z : "<<lcl_point.z<<endl;
			
			if(lcl_count % 1 ==0){
				line_lcl.points.push_back(lcl_point);
			}
